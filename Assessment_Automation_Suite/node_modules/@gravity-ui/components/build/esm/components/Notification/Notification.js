...
function renderSourceTitle({ title, href, id }) {
    return href ? (React.createElement(Link, { className: b('right-source-title'), href: href, target: "_blank", title: title, id: id }, title)) : (React.createElement("div", { className: b('right-source-title'), title: title, id: id }, title));
}
function renderSourceIcon(source, titleId) {
    const iconElement = getIconElement(source);
    if (!iconElement)
        return null;
    return source.href ? (React.createElement(Link, { href: source.href, target: "_blank", extraProps: { 'aria-labelledby': titleId } }, iconElement)) : (iconElement);
}

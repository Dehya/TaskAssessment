...
const b$6 = cn.block('mobile-logo');
const MobileLogo = ({ text, compact, icon, iconSrc, iconClassName, iconSize = 32, textSize = 20, href = '/', target = '_self', wrapper, onClick, className, }) => {
    ...
    if (iconSrc) {
        ...
        logoIcon = React__default["default"].createElement(uikit.Icon, { data: icon, size: iconSize, className: b$6('icon', iconClassName) });
    }
    let logoTitle;
    if (typeof text === 'function') {
        ...
        logoTitle = (React__default["default"].createElement("span", { className: b$6('title'), style: { fontSize: textSize } }, text));
    }
    const logo = (React__default["default"].createElement(React__default["default"].Fragment, null,
        logoIcon,
        logoTitle));
    return hasWrapper ? (React__default["default"].createElement("div", { className: b$6(null, className), onClick: onClick }, wrapper(logo, compact))) : (React__default["default"].createElement("a", { href: href, target: target, ref: target === '_self' ? undefined : 'noreferrer', className: b$6(null, className), onClick: onClick }, logo));
};
...
const b$5 = cn.block('mobile-header-burger');
...
const b$4 = cn.block('burger-composite-bar');
const Item = ({ item, onItemClick }) => {
    const { icon, type = 'regular', iconSize = MOBILE_HEADER_ICON_SIZE } = item;
    if (type === 'divider') {
        return React__default["default"].createElement("div", { className: b$4('menu-divider') });
    }
    const node = (React__default["default"].createElement("div", { className: b$4('item', { type }), onClick: () => {
        ...
        React__default["default"].createElement("div", { className: b$4('item-icon-place') }, icon && React__default["default"].createElement(uikit.Icon, { data: icon, size: iconSize, className: b$4('item-icon') })),
        React__default["default"].createElement("div", { className: b$4('item-title') }, item.title)));
    if (typeof item.itemWrapper === 'function') {
        return item.itemWrapper(node, item);
    }
    return item.link ? (React__default["default"].createElement("a", { href: item.link, className: b$4('link') }, node)) : (node);
};
Item.displayName = 'Item';
const BurgerCompositeBar = React__default["default"].memo(({ items, onItemClick }) => {
    return (React__default["default"].createElement("nav", { className: b$4() },
        React__default["default"].createElement(uikit.List, { items: items, selectedItemIndex: getSelectedItemIndex(items), itemHeight: getItemHeight, itemClassName: b$4('root-menu-item'), virtualized: false, filterable: false, sortable: false, renderItem: (item) => React__default["default"].createElement(Item, { item: item, onItemClick: onItemClick }) })));
});
...
const b$3 = cn.block('mobile-header-burger-menu');
const BurgerMenu = React__default["default"].memo(({ items = [], renderFooter, modalItem, className, onItemClick }) => {
    ...
    return (React__default["default"].createElement("div", { className: b$3(null, className) },
        ...
        renderFooter && React__default["default"].createElement("div", { className: b$3('footer') }, renderFooter === null || renderFooter === void 0 ? void 0 : renderFooter())));
});
...
const b$2 = cn.block('mobile-overlap-panel');
const OverlapPanel = ({ title, renderContent, className, onClose, action, closeTitle = i18n('overlap_button_close'), visible, topOffset, }) => {
    return (React__default["default"].createElement(Drawer.Drawer, { className: b$2('', { action: Boolean(action) }, className), onVeilClick: onClose, onEscape: onClose, preventScrollBody: true, style: {
            top: topOffset,
        } },
        React__default["default"].createElement(Drawer.DrawerItem, { id: "overlap", visible: visible, className: b$2('drawer-item') },
            React__default["default"].createElement("div", { className: b$2('header') },
                React__default["default"].createElement(uikit.Button, { size: "l", view: "flat", className: b$2('close'), onClick: onClose, extraProps: {
                        'aria-label': closeTitle,
                    } },
                    React__default["default"].createElement(uikit.Icon, { className: b$2('icon'), data: icons.ArrowLeft, size: MOBILE_HEADER_ICON_SIZE })),
                React__default["default"].createElement(uikit.Text, { whiteSpace: "nowrap", ellipsis: true, variant: 'subheader-2', className: b$2('title'), as: title ? 'h2' : undefined }, title),
                action && (React__default["default"].createElement(uikit.Button, { size: "l", type: "button", view: "flat", onClick: action.onClick, className: b$2('action'), extraProps: {
                        'aria-label': action.title,
                    } },
                    React__default["default"].createElement(uikit.Icon, { data: action.icon, size: MOBILE_HEADER_ICON_SIZE })))),
            React__default["default"].createElement("div", { className: b$2('content') }, renderContent()))));
};
...
const b$1 = cn.block('mobile-header');
const MobileHeader = React__default["default"].forwardRef(({ logo, burgerMenu, burgerCloseTitle = i18n('burger_button_close'), burgerOpenTitle = i18n('burger_button_open'), panelItems = [], renderContent, sideItemRenderContent, onClosePanel, onEvent, className, contentClassName, overlapPanel, }, ref) => {
    ...
    return (React__default["default"].createElement("div", { className: b$1({ compact }, className), ref: targetRef },
        React__default["default"].createElement("header", { className: b$1('header'), style: { height: size } },
            ...
            React__default["default"].createElement(MobileLogo, Object.assign({}, logo, { compact: compact, onClick: onLogoClick })),
        ...
        React__default["default"].createElement(Drawer.Drawer, { className: b$1('panels'), onVeilClick: onCloseDrawer, onEscape: onCloseDrawer, style: { top: size } }, [burgerPanelItem, ...panelItems].map((item) => (React__default["default"].createElement(Drawer.DrawerItem, Object.assign({}, item, { key: item.id, visible: visiblePanel === item.id, className: b$1('panel-item', item.className) }))))),
        overlapPanel && (React__default["default"].createElement(OverlapPanel, { topOffset: size, className: b$1('overlap-panel'), title: overlapPanel.title, onClose: onOverlapClose, action: overlapPanel.action, visible: overlapPanelVisible, renderContent: overlapPanel.renderContent })),
        React__default["default"].createElement(Content.Content, { size: size, renderContent: renderContent, className: b$1('content', contentClassName), cssSizeVariableName: "--mobile-header-size" })));
});
...
//# sourceMappingURL=FooterItem-003eb137.js.map

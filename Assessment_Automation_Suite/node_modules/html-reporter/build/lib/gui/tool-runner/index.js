...
class ToolRunner {
    ...
    async _handleRunnableCollection() {
        ...
        for (const test of this._ensureTestCollection().tests) {
            ...
            if (test.pending) {
                queue.add(async () => reportBuilder.addTestResult(test.createTestResult({ status: constants_1.SKIPPED })));
            }
            else {
                queue.add(async () => reportBuilder.addTestResult(test.createTestResult({ status: constants_1.IDLE })));
            }
        }
        await queue.onIdle();
